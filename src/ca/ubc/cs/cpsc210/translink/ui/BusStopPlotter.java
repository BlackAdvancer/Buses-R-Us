...
public class BusStopPlotter extends MapViewOverlay {
    ...
    public void markStops(Location currentLocation) {
        ...
        if (nearestStnMarker!= null) {
            ...
            np = (Stop) nearestStnMarker.getRelatedObject();
        }
        for(Stop s : StopManager.getInstance()){
            if(rectangleContainsPoint(super.northWest,super.southEast,s.getLocn()) && !s.equals(np)){
                Marker temp = new Marker(mapView);
                temp.setPosition(Geometry.gpFromLL(s.getLocn()));
                temp.setRelatedObject(s);
                temp.setIcon(stopIconDrawable);
                temp.setTitle(s.getNumber()+" "+s.getName());
                temp.setInfoWindow(stopInfoWindow);
                setMarker(s, temp);
                stopClusterer.add(temp);
            }
        }
    }
    ...
    public void updateMarkerOfNearest(Stop nearest) {
        ...
        clearMarker(nearest);
        Marker temp = new Marker(mapView);
        GeoPoint p = new GeoPoint(nearest.getLocn().getLatitude(),nearest.getLocn().getLongitude());
        temp.setPosition(p);
        temp.setRelatedObject(nearest);
        temp.setIcon(closestStopIconDrawable);
        temp.setTitle(nearest.getNumber()+" "+nearest.getName());
        temp.setInfoWindow(stopInfoWindow);
        setMarker(nearest,temp);
        nearestStnMarker = temp;
